...
HISTSIZE=32768 #DEFAULT: 1000
HISTFILESIZE=$HISTSIZE #DEFAULT: 2000
...
prompt_git() {
    ...
    if [[ $git_state ]]; then
        git_info="$git_info${RS}[$git_state${RS}]"
    fi
    printf "${FWHT} on ${style_branch}${git_info}"
}
...
if [ "$color_prompt" = yes ]; then
    ...
    PS1='${debian_chroot:+($debian_chroot)}\u:\w$(prompt_git)\n\$ '
...
__conda_setup="$('/home/pierre/anaconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/home/pierre/anaconda3/etc/profile.d/conda.sh" ]; then
        . "/home/pierre/anaconda3/etc/profile.d/conda.sh"
    else
